...
import { matrix, matrix1, matrix2, matrix3 } from './matrix';
...
export class Calc {
    ...
    constructor1(alts: Alts[],
        data: matrix,
        vol: matrix1,
        weight: vector1,
        seg: vector,
        seg_index: number,
        number_of_saved_combos: number,
        itemStr: any = [],
        threshold: number){
        var intArr = [];
        const obj ={
        ...
        for (var i = 1; i <= itemStr.length; i++) {
            var v1 = itemStr[i-1];
            var v2 = v1.substring(4, v1.length - 0)
            //console.log(36,v2);
            const obj ={
                id: parseInt(v2),
                name: v1,
        ...
        for (var i = 1; i <= this.l_data.rowCount; i++) {
            ...
            for (var j = 1; j <= this.l_data.columnCount; j++) {
                if (data._matrix[i - 1][j - 1] >= threshold && j == 20)
                    this.l_data._matrix[i - 1][j - 1] = 1;
                else
                    this.l_data._matrix[i - 1][j - 1] = 0;
            }
        }
        console.log(102,this.l_data._matrix);
    ...
    CalculateParallel1(items : any[],allow_fixed_items:boolean): any[] {
        var results : any[] =[];
        let total_num_iteration: number;
    ...
    CalculateParallel2(num_chosen: number, allow_fixed_items: boolean): any[] {
        var results : any[] =[];
        let total_num_iteration: number;
        //if(num_chosen > 8){}
        for(let num = 1; num <=num_chosen; num++){
