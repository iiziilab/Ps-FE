...
@Component({
  selector: 'app-twoitems',
  templateUrl: './twoitems.component.html',
  styles: [`
  .btnrptcontainer{
    display:grid;
  }
...
export class TwoitemsComponent implements OnInit {

  id:any;
  displayedColumns: string[] = ['Sr.No','Reach','Frequency','Freqreach','Item1','Item2','Incremental_reach1','Incremental_reach2'];
  ...
  async ngOnInit(): Promise<void> {
    ...
    // for(let c= 0;c<this.result[0][1].length;c++) {
    //   const pp : any={
    //     reach:this.result[0][1][c],
    //     frequency:this.result[0][3][c],
    //     item:this.result[0][2][c][0]
    //   }
    //   this.firstlist.push(pp);
    // };
    // for(let c= 0;c<this.result[1][1].length;c++) {
    //   var ID = this.firstlist.filter(x => x.item === this.result[1][2][c][0])
    //   const p : twoitem={
    //     reach:this.result[1][1][c],
    //     frequency:this.result[1][3][c],
    //     item:this.result[1][2][c][0]+','+this.result[1][2][c][1],
    //     incremental_reach1 : ID[0].reach,
    //     incremental_reach2 : (this.result[1][1][c]-ID[0].reach).toFixed(3)
    //   }
    //   this.oneitemlist.push(p);
    // };
    //this.itemlist=this.exportlist.itemreport();
    console.log(152,this.itemlist[1]);
    this.dataSource = new MatTableDataSource(this.itemlist[1]);

    this.pName = localStorage.getItem('projectName');
    this.cellName = project.cellName;
    //console.log(155,this.itemlist[1]);
  }
  ...
  generateExcel() {
    ...
    var itemlist: any[] = [];
    itemlist = JSON.parse(localStorage.getItem('report'));
    //itemlist=this.exportlist.itemreport();
    for (let d = 0; d < this.arrlength; d++) {
