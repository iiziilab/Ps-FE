...
import { role, Userlog } from 'src/app/model/userlog.model';
...
export class AdminInfoComponent implements OnInit {
  @Output() names = new EventEmitter<any>();
  ...
  constructor(private services: PsolutionsService,
    ...
    if (this.authenticationService.currentUserValue) {
      ...
      this.email = this.authenticationService.currentUserValue.data.email;
      this.role = this.authenticationService.currentUserValue.data.role;
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this.logid) {
      this.save = false;
      this.edit = true;
      const result = await this.services.getUserInfoByid(this.logid).toPromise();
      ...
      this.f.designation.setValue(this.design);
      this.f.roleId.setValue(result.roleId);
      this.f.statusId.setValue(result.statusId);
      this.f.password.setValue(result.password);
      await this.services.getUserInfoPicbyId(this.logid).toPromise().then(x=>{
        if(x.statusCode == 200){
  ...
  async onEdit(): Promise<void> {
    ...
    try {
      ...
      const result = await this.services.updateUserInfo(this.logid, model).toPromise();
      if (result.statusCode == 200) {
        await this.uploadFile();
        this.fwdMsgToSib2(this.image);
  ...
  handleFileInput(e: any) {
    ...
    if (files && files.length > 0) {
      ...
      this.adminform.get('pic').setValue(this.fileToUpload);
      console.log(this.adminform.get('pic').value);
    }

    const file = (e.target as HTMLInputElement).files[0];
    this.adminform.get('pic').updateValueAndValidity()
  ...
  async uploadFile() : Promise<void> {
    ...
    await this.services.postUserFile(this.logid,formModel).subscribe(x => {
       console.log(x);
       this.image = 'data:image/jpeg;base64,' + x.data.image;
  ...
  onFileChange(event:any) {
    if(event.target.files.length > 0) {
      ...
      this.adminform.get('pic').setValue(file);
    }
  }

  private prepareSave(): any {
    let input = new FormData();
    input.append('file', this.adminform.get('pic').value);
    return input;
  }

  clearFile() {
    this.adminform.get('avatar').setValue(null);
    this.fileInput.nativeElement.value = '';
  }

  // msgToSib() { this.msgTolayout.emit(this.image)}
